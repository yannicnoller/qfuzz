...
#define DEFAULT_MAX_NUM_CLUSTERS 0

/* YN: +24 for resource results +8 for user defined cost +4 number of clusters +8 for minimum delta value (cluster values) = +44 */
static int TOTAL_TRACE_BITS_SIZE = SHM_SIZE+44;
...
void printUsageAndDie() {
  DIE("Usage: interface [-s <server>] [-p <port>] [-c <max_number_of_clusters>] <filename>\n");
}
...
int main(int argc, char** argv) {
  ...
  int max_number_of_clusters = DEFAULT_MAX_NUM_CLUSTERS;
  ...
  port = argv[curArg+1];
  curArg += 2;
      } else if (argv[curArg][1] == 'K') { // YN: cluster
        sscanf (argv[curArg+1], "%d", &max_number_of_clusters);
        TOTAL_TRACE_BITS_SIZE = TOTAL_TRACE_BITS_SIZE + max_number_of_clusters * 8;
        curArg += 2;
      } else {
        LOG("Unkown flag: %s\n", argv[curArg]);
  printUsageAndDie();
  ...
  do {
    ...
    /* YN: Define arrays for cluster and delta values. */
    double cluster_values[max_number_of_clusters];
...
//    }
    ...
    /* YN: Receive observation cluster information. */
    int number_of_clusters;
    nread = read(tcp_socket, &number_of_clusters, 4);
    if (nread != 4) {
      LOG("Failed to read number_of_clusters\n");
      status = STATUS_COMM_ERROR;
      goto cont;
    }
    LOG("#Clusters: %d\n", number_of_clusters);
    ...
    nread = read(tcp_socket, &min_delta_value, 8);
    if (nread != 8) {
      LOG("Failed to read min_delta_value\n");
      status = STATUS_COMM_ERROR;
      goto cont;
    }
    LOG("minDelta: %.2lf\n", min_delta_value);
    ...
    for (int i=0; i<number_of_clusters; i++) {
      ...
      nread = read(tcp_socket, &cluster_value, 8);
      if (nread != 8) {
        LOG("Failed to read cluster_value\n");
        status = STATUS_COMM_ERROR;
        goto cont;
      }
      cluster_values[i] = cluster_value;
      LOG("Cluster value %d: %.2lf\n", i, cluster_value);
    }
    ...
    /* YN: cluster information */
    memcpy(&trace_bits[SHM_SIZE+32], &number_of_clusters, 4);
    memcpy(&trace_bits[SHM_SIZE+36], &min_delta_value, 8);
    for (int i=0; i<number_of_clusters; i++) {
      memcpy(&trace_bits[SHM_SIZE+44+(i*8)], &cluster_values[i], 8);
    }
