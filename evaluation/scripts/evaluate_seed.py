"""
    ...
    python3 evaluate_seed.py /Users/yannic/experiments/qfuzz-new-results apache_wss4j_KGreedy 5 30 1800 3
...
if __name__ == '__main__':
        ...
        for i in range(START_INDEX, NUMBER_OF_EXPERIMENTS+1):
            ...
            with open(dataFile,'r') as csvfile:
                ...
                for row in csvreader:
                    ...
                    if dataId not in time_partition_greater_one and currentPartition > 1:
    ...
    with open(outputFileName, "w") as csv_file:
        ...
        if len(time_values) == NUMBER_OF_EXPERIMENTS*NUMBER_OF_SEEDS:
            avg_time = "{0:.2f}".format(sum(time_values)/float(NUMBER_OF_EXPERIMENTS*NUMBER_OF_SEEDS))
            error = "{0:.2f}".format(1.960 * numpy.std(time_values)/float(math.sqrt(NUMBER_OF_EXPERIMENTS*NUMBER_OF_SEEDS)))

            # Mean time to local maximum partition (might not be a valid metric.)
            mean_time_local_max = "{0:.2f}".format(sum(times_local_maximum)/float(NUMBER_OF_EXPERIMENTS*NUMBER_OF_SEEDS))
            error_mean_time_local_max = "{0:.2f}".format(1.960 * numpy.std(times_local_maximum)/float(math.sqrt(NUMBER_OF_EXPERIMENTS*NUMBER_OF_SEEDS)))

            # Mean time to absolute maximum partition (at least EXPERIMENT_TIMEOUT).
            mean_time_absolute_max = "{0:.2f}".format(sum(times_absolute_maximum)/float(NUMBER_OF_EXPERIMENTS*NUMBER_OF_SEEDS))
            error_mean_time_absolute_max = "{0:.2f}".format(1.960 * numpy.std(times_absolute_maximum)/float(math.sqrt(NUMBER_OF_EXPERIMENTS*NUMBER_OF_SEEDS)))
